...
from Bot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("./Bot/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="[TU](https://t.me/gangs_for_udanpirappu)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./Bot/resources/fonts"):
            os.remove(font_)
...
async def logo_(event):
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [TU](https://t.me/gangs_for_udanpirappu)",
            force_document=False,
        )
        os.remove(flnme)
        await xx.delete()
